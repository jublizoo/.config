...
## Type: FormatString
## Use id + 1 because for some reason there is an off-by-one.
...
def _one_line_js(s: str) -> str:
    return " ".join(s.split())

JS_SCROLL_TEMPLATE = r"""
(function (dir) {
  let el = document.elementFromPoint(innerWidth/2, innerHeight/2);
  const step = innerHeight * 0.5 * dir;
  ...
  while (el) {
    ...
    el = (el && el.parentElement) ||
         (el && el.getRootNode && el.getRootNode().host) ||
         null;
  }
...
def _scroll_js(dir_sign: int) -> str:
    return _one_line_js(JS_SCROLL_TEMPLATE.replace("DIR", str(dir_sign)))

config.bind('<Ctrl-d>', 'jseval -q ' + _scroll_js(+1))
config.bind('<Ctrl-u>', 'jseval -q ' + _scroll_js(-1))
config.bind('<Ctrl-e>', 'jseval -q ' + _scroll_js(+0.5))
config.bind('<Ctrl-y>', 'jseval -q ' + _scroll_js(-0.5))
